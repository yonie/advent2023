...
rl.on('line', game => {
  const gameId = Number(game.substring(5).split(':')[0])
  const [possible, power] = isGamePossible(game)
...
function isGamePossible (game) {
  ...
  sets.forEach(set => {
    ...
    parts.forEach(part => {
      const numberOfDice = Number(part.split(' ')[0])
      const diceColor = part.split(' ')[1]
  ...
  return [
    possible,
    minimumDiceNeeded.red * minimumDiceNeeded.blue * minimumDiceNeeded.green
  ]
}
